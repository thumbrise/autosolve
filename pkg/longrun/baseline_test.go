...
func TestExponential_InBaselineContext(t *testing.T) {
	backoff := longrun.Exponential(2*time.Second, 2*time.Minute)
...
func TestNewBaseline_SetsNodeAndService(t *testing.T) {
	node := longrun.Policy{Backoff: longrun.Exponential(1*time.Second, 1*time.Minute)}
	service := longrun.Policy{Backoff: longrun.Exponential(2*time.Second, 2*time.Minute)}
...
func TestNewBaselineDegraded_SetsDefault(t *testing.T) {
	node := longrun.Policy{Backoff: longrun.Exponential(1*time.Second, 1*time.Minute)}
	service := longrun.Policy{Backoff: longrun.Exponential(2*time.Second, 2*time.Minute)}
	degraded := longrun.Policy{Backoff: longrun.Exponential(5*time.Second, 5*time.Minute)}
	...
	// BackoffFunc is opaque — verify via calling it.
	if d := b.Default.Backoff(0); d != 5*time.Second {
		t.Fatalf("expected Default.Backoff(0) = 5s, got %v", d)
	}
}
